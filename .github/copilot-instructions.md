...
- **Rebuild**:
  ...
  ```
- **Access**: Main app via Nginx at `http://localhost:81`, Backend directly at `http://localhost:8000`
...
- **Repo Root**: `README.md`, `docker-compose.yaml`, `nginx.conf`, `azure-pipelines.yml`, `.sonarcloud.properties`
- **Server/**: Backend code, `main.py`, `pyproject.toml`, `alembic.ini`, `db/`, `app/`, `scripts/`, `test_custom_logging.py`
- **Webapp/**: Frontend code, `package.json`, `eslint.config.mjs`, `jest.config.js`, `src/`, `test/`
- **Timer/**: Raspberry Pi timer, `pyproject.toml`, `src/timer.py`, `install_timer.sh`, `README.md`
- **Common/**: `openapi.json` (shared API schema)
- **docs/**: `architecture.md`, `deployment/server_setup_guide.md`, `decisions/ADR*.md`, diagrams
