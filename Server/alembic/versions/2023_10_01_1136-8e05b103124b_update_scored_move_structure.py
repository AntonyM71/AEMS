...
def upgrade() -> None:
    ...
    op.drop_table('run')
    op.alter_column('competition', 'id',
               existing_type=postgresql.UUID(),
               comment=None,
               existing_comment='Competition ID',
               existing_nullable=False)
    op.alter_column('scoredBonuses', 'id',
               existing_type=postgresql.UUID(),
               comment=None,
               existing_comment='Competition ID',
               existing_nullable=False)
    op.add_column('scoredMoves', sa.Column('heat_id', postgresql.UUID(as_uuid=True), nullable=True))
    op.add_column('scoredMoves', sa.Column('run_number', sa.String(), nullable=False))
    op.add_column('scoredMoves', sa.Column('athlete_id', postgresql.UUID(as_uuid=True), nullable=False))
    op.add_column('scoredMoves', sa.Column('direction', sa.String(), nullable=False))
    op.alter_column('scoredMoves', 'id',
               existing_type=postgresql.UUID(),
               comment=None,
               existing_comment='Competition ID',
               existing_nullable=False)
...
def downgrade() -> None:
    # ### commands auto generated by Alembic - please adjust! ###
    op.add_column('scoredMoves', sa.Column('run_id', postgresql.UUID(), autoincrement=False, nullable=True))
    op.drop_constraint(None, 'scoredMoves', type_='foreignkey')
    op.drop_constraint(None, 'scoredMoves', type_='foreignkey')
    op.create_foreign_key('scoredMoves_run_id_fkey', 'scoredMoves', 'run', ['run_id'], ['id'])
    op.alter_column('scoredMoves', 'id',
               existing_type=postgresql.UUID(),
