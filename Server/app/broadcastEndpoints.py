...
async def timer_websocket(websocket: WebSocket) -> None:
    ...
    try:
        ...
        logging.info("WebSocket /timer: disconnect reason: %s, code: %s",
                     getattr(e, 'reason', None), getattr(e, 'code', None))
...
async def broadcast_control_websocket(websocket: WebSocket) -> None:
    ...
    try:
        ...
        logging.info("WebSocket /broadcast_control: disconnect reason: %s, code: %s",
                     getattr(e, 'reason', None), getattr(e, 'code', None))
        if websocket.client_state == WebSocketState.CONNECTED:
            await websocket.close()
    except Exception as e:
        logging.exception("Broadcast control WebSocket error: %s", e)
    finally:
        logging.info(
            "WebSocket /broadcast_control: entering finally block, client_state=%s", websocket.client_state)
        if websocket.client_state == WebSocketState.CONNECTED:
            await websocket.close()
