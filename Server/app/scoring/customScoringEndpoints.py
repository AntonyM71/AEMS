...
@scoring_router.post("/upsert_run_status/")
def upsert_run_status(run_status: RunStatusSchema, db: Session = Depends(get_transaction_session)):
    existing_run_status = db.query(RunStatus).filter(
    ...
    if existing_run_status:
        ...
        return {"message": "Run status updated", "run_status": existing_run_status}
    else:
        ...
        return {"message": "Run status created", "run_status": new_run_status}
