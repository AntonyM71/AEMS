            ...
            if str(clause.left).endswith(".score"):
                ...
                if clause.operator.__name__ == "ge":
                    ...
                    assert clause.right.value == 5, (
                        f"Expected 5 for score____from, got {clause.right.value}"
                    )
                # Check for <= (le) operator for score____to
                    ...
                    assert clause.right.value == 15, (
                        f"Expected 15 for score____to, got {clause.right.value}"
                    )
    else:
        if str(whereclause.left).endswith(".score"):
            found_score_from = True

    assert found_score_from, (
        "Expected to find score____from (>=) filter in WHERE clause"
    )
