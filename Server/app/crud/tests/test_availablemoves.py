...
@pytest.fixture
def mock_available_moves(mock_available_moves_data: dict[str, Any]) -> AvailableMoves:
...
def test_get_many_available_moves_with_sheet_id_filter(
    ...
    # Assert we're filtering on the correct column
    assert str(whereclause.left).endswith(".sheet_id"), (
        f"Expected filtering on .sheet_id column, got {whereclause.left}"
    )
    ...
    # Assert the actual filter value matches what we sent in the request
    filter_values = whereclause.right.value
    assert any(str(val) == filter_sheet_id for val in filter_values), (
        f"Expected {filter_sheet_id} in filter values, got {filter_values}"
    )
...
def test_get_many_available_moves_with_fl_score_range(
    ...
    response = test_client.get(
        "/availablemoves/?fl_score____from=50&fl_score____to=150"
    )
        ...
        for clause in whereclause.clauses:
            if str(clause.left).endswith(".fl_score"):
                ...
                if clause.operator.__name__ == "ge":
                    ...
                    assert clause.right.value == 50, (
                        f"Expected 50 for fl_score____from, got {clause.right.value}"
                    )
                # Check for <= (le) operator for fl_score____to
                    ...
                    assert clause.right.value == 150, (
                        f"Expected 150 for fl_score____to, got {clause.right.value}"
                    )
    else:
        if str(whereclause.left).endswith(".fl_score"):
            found_fl_score_from = True

    assert found_fl_score_from, (
        "Expected to find fl_score____from (>=) filter in WHERE clause"
    )
    assert found_fl_score_to, (
        "Expected to find fl_score____to (<=) filter in WHERE clause"
    )
        ...
        for clause in whereclause.clauses:
            if str(clause.left).endswith(".rb_score"):
                ...
                if clause.operator.__name__ == "ge":
                    ...
                    assert clause.right.value == 40, (
                        f"Expected 40 for rb_score____from, got {clause.right.value}"
                    )
                # Check for <= (le) operator for rb_score____to
                    ...
                    assert clause.right.value == 60, (
                        f"Expected 60 for rb_score____to, got {clause.right.value}"
                    )
    else:
        if str(whereclause.left).endswith(".rb_score"):
            found_rb_score_from = True

    assert found_rb_score_from, (
        "Expected to find rb_score____from (>=) filter in WHERE clause"
    )
    assert found_rb_score_to, (
        "Expected to find rb_score____to (<=) filter in WHERE clause"
    )
...
def test_get_many_available_moves_with_direction_filter(
    ...
    # Assert we're filtering on the correct column
    assert str(whereclause.left).endswith(".direction"), (
        f"Expected filtering on .direction column, got {whereclause.left}"
    )
    ...
    filter_direction = "U"
    filter_values = whereclause.right.value
    assert filter_direction in filter_values, (
        f"Expected {filter_direction} in filter values, got {filter_values}"
    )
