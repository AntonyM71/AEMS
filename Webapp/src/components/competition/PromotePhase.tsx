...
export const PromotePhase = () => {
	...
	return (
		<Paper sx={{ padding: "1em", height: "100%" }}>
			<Grid container spacing="1em" width="50%" alignSelf="center">
				<Grid item xs={12}>
				...
				{newHeatNames.map((h: string, i) => (
					<Grid item key={i} xs={12}>
						<TextField
							...
							error={
								!h ||
								newHeatNames.filter((j) => j === h).length !== 1
							}
							helperText={
								!h
							...
							onChange={(
								...
								const updatedNewHeatNames = [...newHeatNames]
								updatedNewHeatNames[i] = event.target.value
