...
Behavior changes go through [OpenSpec](https://github.com/Fission-AI/OpenSpec), not straight to code. Propose a change (`/opsx:propose` in Claude Code, `$openspec-propose` in Codex, `/opsx-propose` in GitHub Copilot), which writes a proposal, a spec delta under `openspec/changes/<name>/specs/`, and a task list; get it reviewed; implement via `/opsx:apply`; then `/opsx:archive` to merge the delta into the canonical spec under `openspec/specs/`. This supersedes the older ad-hoc `docs/superpowers/specs` + `docs/superpowers/plans` workflow (removed — its still-relevant content was folded into `openspec/specs/` and new ADRs) — `openspec/specs/` is now the source of truth for what the system must do. ADRs in [docs/decisions/](docs/decisions/) are unaffected: they record *why* a past decision was made, OpenSpec records *what* the system does now — don't duplicate one in the other. An ADR records the decision and the options weighed against it, so keep implementation out of it: no SQL, no schema or column names, no code snippets, no pinned dependency versions, no file paths. Those belong in the change's `design.md`, which is where an implementing agent looks for them, and which won't drift out of step with the code the way a second copy in an ADR does.

When writing or reviewing a spec, check GitHub issues and commit-message bodies touching that capability's code — issues here often describe a problem or desired behavior in more detail than the matching PR (e.g. #393's Summary/Proposal), and commit bodies frequently explain the "why" behind an edge case. Before folding one in, check whether it's already shipped: an **open** issue describing behavior nobody has built yet (e.g. #434, asking tie-break messages to name every tied athlete instead of just the adjacent pair) belongs in a future change's `MODIFIED Requirements` once implemented — never backfilled straight into a baseline spec, which must describe what the system actually does today, not what an issue wants it to do.
