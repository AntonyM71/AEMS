	...
	scoredMoves.map((scoredMove) => {
		...
		if (filteredMoves.length === 1) {
			...
			if (
                ...
                const recentMove = calculateScoreWithBonuses(scoredMoveObject, scoredMove)
				...
				// eslint-disable-next-line no-console
                console.log("Move " + scoredMoveObject.name + " already scored higher version of sme move")
                }
			} else {
				if (scoredMoveObject.score[scoredMove.direction]) {
					scoresArray.push(calculateScoreWithBonuses(scoredMoveObject, scoredMove))
...
const calculateScoreWithBonuses = (scoredMoveObject: movesType, scoredMove: scoredMovesType) => {
